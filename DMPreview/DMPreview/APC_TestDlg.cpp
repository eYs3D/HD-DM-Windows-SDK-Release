...
// CAPC_TestDlg 對話方塊
CAPC_TestDlg::CAPC_TestDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CAPC_TestDlg::IDD, pParent)
{
...
void CAPC_TestDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_DEVCOMBO, m_Device_Ctrl);
}

BEGIN_MESSAGE_MAP(CAPC_TestDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(ID_OPEN_DEVICE, &CAPC_TestDlg::OnBnClickedOpenDevice)
    ON_MESSAGE(WM_MSG_CLOSE_VIDEO_DEVICE_DLG, &CAPC_TestDlg::OnCloseVideoDeviceDlg)
    ON_WM_CLOSE()
	ON_MESSAGE(WM_MSG_SNAPSHOT_ALL, OnShapShotAll)
END_MESSAGE_MAP()

LRESULT CAPC_TestDlg::OnShapShotAll(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CAPC_TestDlg::OnCloseVideoDeviceDlg(WPARAM wParam, LPARAM lParam)
{
...
BOOL CAPC_TestDlg::OnInitDialog() {
  ...
  SetWindowText(str);
  if (!GetApcDIDevice(m_devInfo))
  {
      AfxMessageBox(L"APC_Init failed ..");
      EndDialog(0);  
      return FALSE;
  }
...
void CAPC_TestDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CAPC_TestDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}


void CAPC_TestDlg::OnBnClickedOpenDevice() {

    int devIndex = m_Device_Ctrl.GetCurSel();
	OpenDevice(devIndex);
}

void CAPC_TestDlg::OpenDevice(int devIndex)
{
	if (m_pVideoDevDlg[devIndex] == NULL)
    {
        CString str;
